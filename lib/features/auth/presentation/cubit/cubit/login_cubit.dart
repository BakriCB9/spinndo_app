...
class LoginCubit extends Cubit<LoginState> {
  LoginCubit(this._loginUseCase,this._resetPassword) : super(LoginState());
 ...
 final ResetPasswordUseCase _resetPassword;
  Future<void> login(LoginRequest requestData) async {
    ...
    result.fold(
      ...
      (response) => emit(state.copyWith(loginStatus: BaseSuccessState<Data>(data:response))),
    );
  }
...
Future<void> resetPassword(ResetPasswordRequest requestData) async {
    ...
    result.fold(
      (failure) => emit(state.copyWith(resetStatus: BaseErrorState(failure.message))),
